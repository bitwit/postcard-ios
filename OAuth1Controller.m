...
#define OAUTH_CALLBACK       @"http://www.postcardsocial.net" //Sometimes this has to be the same as the registered app callback url
#define CONSUMER_KEY         @"D9uLIBJbocJDyKejzw6R4tuVexkkCDNt3iER7d3mkEMveRwBMG"
#define CONSUMER_SECRET      @"EpJLHK7FC9H3OOYfpdM0UoozLsEmmzzahmdTIjfWvJ5c8HXHQe"
...
- (void)obtainRequestTokenWithCompletion:(void (^)(NSError *error, NSDictionary *responseParams))completion
{
    ...
    NSString *baseString = [REQUEST_TOKEN_METHOD stringByAppendingFormat:@"&%@&%@", request_url.encodeForURL, parametersString.encodeForURL];
    NSString *secretString = [oauth_consumer_secret.encodeForURL stringByAppendingString:@"&"];
    ...
    for (NSString *name in allParameters) {
        NSString *aPair = [name stringByAppendingFormat:@"=\"%@\"", [allParameters[name] encodeForURL]];
...
- (void)authenticateToken:(NSString *)oauthToken withCompletion:(void (^)(NSError *error, NSDictionary *responseParams))completion
{
    ...
    authenticate_url = [authenticate_url stringByAppendingFormat:@"&oauth_callback=%@", oauth_callback.encodeForURL];
                ...
                completion:(void (^)(NSError *error, NSDictionary *responseParams))completion
{
    ...
    NSString *baseString = [ACCESS_TOKEN_METHOD stringByAppendingFormat:@"&%@&%@", access_url.encodeForURL, parametersString.encodeForURL];
    NSString *secretString = [oauth_consumer_secret.encodeForURL stringByAppendingFormat:@"&%@", oauth_token_secret.encodeForURL];
    NSString *oauth_signature = [self.class signClearText:baseString withSecret:secretString];
    [allParameters setValue:oauth_signature forKey:@"oauth_signature"];
                             ...
                             oauthSecret:(NSString *)oauth_token_secret
{
    ...
    NSString *oauth_consumer_secret = CONSUMER_SECRET;
    NSString *baseString = [HTTPmethod stringByAppendingFormat:@"&%@&%@", request_url.encodeForURL, parametersString.encodeForURL];
    NSString *secretString = [oauth_consumer_secret.encodeForURL stringByAppendingFormat:@"&%@", oauth_token_secret.encodeForURL];
    NSString *oauth_signature = [self.class signClearText:baseString withSecret:secretString];
